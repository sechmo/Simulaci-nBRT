...
to setup
  ...
  crear-estacion patch -500 0 100
  crear-estacion patch -150 0 100
  crear-estacion patch 200 0 100
  crear-estacion patch 550 0 100
  crear-estacion patch 900 0 100


  ; Crear calles
  ask patches with [ abs(pycor) > 5 and abs(pycor) < 30 ] [
    set tipo "calle"
    set pcolor black
  ]
...
to setup2
  ...
  ; Portal
  crear-estacion patch -800 0 300


  ; Crear calles
  crear-calle (patch 0 9) (patch min-pxcor 9) 6 "primera" black
  crear-calle (patch 0 9) (patch max-pxcor 9) 6 "primera" black
  crear-calle (patch 0 -9) (patch min-pxcor -9) 6 "primera" black
  crear-calle (patch 0 -9) (patch max-pxcor -9) 6 "primera" black

  crear-calle (patch 0 17) (patch min-pxcor 17) 6 "segunda" black
  crear-calle (patch 0 17) (patch max-pxcor 17) 6 "segunda" black
  crear-calle (patch 0 -17) (patch min-pxcor -17) 6 "segunda" black
  crear-calle (patch 0 -17) (patch max-pxcor -17) 6 "segunda" black
  ; Buses para cada estacion
  crear-bus (patch -150 9) 2 VelocidadBuses 90 [self] of up-to-n-of 3 paradas with [orientacion-parada = 1]
  crear-bus (patch -100 9) 2 VelocidadBuses 90 [self] of up-to-n-of 3 paradas with [orientacion-parada = 1]
  crear-bus (patch -100 17) 2 VelocidadBuses 90 [self] of up-to-n-of 3 paradas with [orientacion-parada = 1]
  crear-bus (patch 150 -9) 2 VelocidadBuses -90 [self] of up-to-n-of 3 paradas with [orientacion-parada = -1]
  ; Pasajeros aleatorios en funcion de densidad
  ask patches with [tipo = "piso"] [
    set interesados []
...
to crear-linea [final tipo-linea main-co]
  set tipo tipo-linea
  set pcolor main-co
  let siguiente patch-at-heading-and-distance (towards final) (1 + (towards final mod 45) / 45 * 0.5)
  if siguiente != final [
    ask siguiente [
      crear-linea final tipo-linea main-co
    ]
  ]
end
to crear-calle [inicio final ancho tipo-linea main-co]
  let d (- floor (ancho / 2))
  let angulo 0
  while [d < floor (ancho / 2) + 1] [
    set angulo [(towards final + 90)] of inicio
    ask [patch-at-heading-and-distance angulo d] of inicio [
      crear-linea ( [patch-at-heading-and-distance angulo d] of final ) (word tipo-linea ifelse-value (d = 0) ["-principal"] [""]) (main-co + ifelse-value ( d = 0 ) [2] [0])
    ]
   set d d + 1
  ]
end
...
; ESTACIONES
; ----------------------------------
to crear-estacion [origen posible-ancho]
  ...
  let ancho 10
  let largo ifelse-value (posible-ancho > 50) [posible-ancho] [100]
...
to dibujar-portal
  ask patches with [((abs pycor) <= 10 and pxcor <= -617  and pxcor >= -936)][
    set pcolor gray - 1
  ]
  ask patches with[((abs pycor) <= 3 and pxcor <= -622 and pxcor >= -931 )][
    set pcolor gray - 2
  ]
  ask patches with [(abs pycor) <= 10 and (pxcor >= -1000 and pxcor <= -937)][
    set pcolor black
    set tipo "calle"
  ]
  ask patches with [(abs pycor) >= 28 and (abs pycor) <= 43 and pxcor <= -617  and pxcor >= -936][
    set pcolor gray - 1
  ]
  ask patches with [(abs pycor) >= 33 and (abs pycor) <= 38 and pxcor <= -622  and pxcor >= -931][
    set pcolor gray - 2
  ]
end
  ...
  ask p [
    ...
    while [c < 21] [
      ask patch-at-heading-and-distance [heading] of myself (- c) [
        let guardar-tipo tipo
;        set estado-anterior tipo
        set tipo ifelse-value estado-patches-bus = "calle" [estado-anterior] [estado-patches-bus]
        set estado-anterior guardar-tipo
...
to-report vagones-cercanos
  let bus-actual self
  let vagones-cer conexiones with [union = 0 or [bus-correspondiente] of union != bus-actual] in-cone 40 150

  report vagones-cer
end


to-report patch-segunda-linea
  let ori orientacion-parada
  let p nobody
  set p one-of patches in-cone 30 90 with [tipo = "segunda-principal"  and distance myself > [velocidad] of myself and orientacion-parada = ori ]
  let c 0
  while [p = nobody] [
    set p one-of patches in-cone 30 90 with [tipo = "segunda-principal"  and distance myself > random-float 1.1 and orientacion-parada = ori ]
    if c > 10 [
      set p min-one-of patches with [tipo = "primera-principal" and abs ([heading] of myself - [towards myself] of myself) < 90 and orientacion-parada = ori ] [distance myself]
    ]
    set c c + 1
  ]
  report p
...
to-report patch-primera-linea
  let p nobody
  set p one-of patches in-cone 30 90 with [tipo = "primera-principal"  and distance myself > [velocidad] of myself ]
  let c 0
  while [ p = nobody ] [
    show "primera"
    set p one-of patches in-cone 30 90 with [tipo = "primera-principal"  and distance myself > random-float 1.1]
    if c > 10 [
      set p min-one-of patches with [tipo = "primera-principal" and abs ([heading] of myself - [towards myself] of myself) < 90 ] [distance myself]
    ]
    set c c + 1
  ]
  report p
...
to-report proxima-parada-bus
  report last paradas-restantes
end

to-report punto-critico
  let distancia-vagones cantidad-vagones * 21
  report [patch-ahead (- (distancia-vagones + 20))] of proxima-parada-bus
end

to-report mas-cercano
  report min-one-of vagones-cercanos [distance myself]
end
to-report avanzar
  let vagon-c mas-cercano
  report vagon-c = nobody or [distance myself] of vagon-c > ifelse-value velocidad > 6 [velocidad] [6]
end

to-report longitud-bus
  report cantidad-vagones * 21
end

to desacelerar [objetivo-referencia opcional]
  let cambio-velocidad (- ( velocidad ^ 2 ) / ( 2 * distance objetivo-referencia ))
  show "yo"
  type (word "con referencia a " objetivo-referencia " la velocidad va a cambiar en " cambio-velocidad)
  ifelse  (velocidad + cambio-velocidad < 0) [;;or velocidad + cambio-velocidad > velocidad) [
    set cambio-velocidad velocidad
  ][
    set velocidad velocidad + cambio-velocidad * opcional
  ]
end


to acelerar
  ifelse velocidad > VelocidadMaxima [
    show "esto va a desacelerar"
    set velocidad velocidad * 0.8
  ][
    set velocidad velocidad + (AceleracionMax * (random-float 1))
  ]
end
to-report objetivo
  let prox      proxima-parada-bus
  let p-critico punto-critico
  let long      longitud-bus
  let tipo-p    [tipo] of patch-here
  let primera   (tipo-p = "primera" or tipo-p = "primera-principal")
  if (abs ycor) > 18 [
    show "objetivo mal"
    report min-one-of patches with [tipo = "segunda-principal" and abs ([heading] of myself - [towards myself] of myself) < 90] [distance myself]
  ]
  ifelse distance prox < long and (abs (heading - towards prox)) < 90 [
    ifelse primera [
      show "objetivo proxima"
      report prox
    ][
      show "objetivo prim"
      report patch-primera-linea
    ]
  ][
    ifelse distance p-critico <= long * 7 / 4 [
      show "objetivo crit"
      report ifelse-value distance p-critico < 4 and (abs (heading - towards prox)) < 45 [prox] [p-critico]
    ][
      show "objetivo segunda"
      report patch-segunda-linea
    ]
  ]
end
to-report tipo-diferente [tipoA tipoB]
  report not is-number? position tipoA tipoB
end

to conducir3
  ifelse estado != "estacionado" [
    ifelse distance proxima-parada-bus < 1.5 [
       set estado "estacionado"
      ask turtle-set vagones-bus [
        estacionar
      ]
      ask proxima-parada-bus [
        set abierta true
        ask turtle-set puertas-parada [
          abrir-puerta
        ]
      ...
      let objetivo-actual objetivo
      show velocidad
      ifelse avanzar [
        show objetivo-actual
        ifelse (objetivo-actual = proxima-parada-bus or  (is-patch? objetivo-actual and ([tipo] of objetivo-actual != [tipo] of patch-here)) or velocidad > VelocidadMaxima) and velocidad > 0 [
          show "desacelero"
          ifelse objetivo-actual = proxima-parada-bus [
            desacelerar objetivo-actual 1
          ][
            ifelse velocidad < 1 [
             acelerar
            ][
            desacelerar objetivo-actual random-float 0.5
            ]
          ]
        ][
          show word "acelero" ifelse-value (is-patch? objetivo-actual) [[tipo] of objetivo-actual != [tipo] of patch-here] [""]
          acelerar
        ]
      ...
      girar towards objetivo-actual
    ]
  ][
    set tiempo-estado tiempo-estado + 1
    let proxima-parada proxima-parada-bus
    if tiempo-estado > TiempoEsperaBus [
      set estado "andando"
      ask turtle-set vagones-bus [
        partir
      ...
      set paradas-restantes but-last paradas-restantes
      set paradas-atendidas fput proxima-parada paradas-atendidas
      if paradas-restantes = [] [
        set paradas-restantes paradas-atendidas
      ]
      set tiempo-estado 0
    ]
  ]
...
"Estacion 1" 1.0 0 -11033397 true "" "plot cuenta-pasajeros-estacion estacion 105"
"Estacion 2" 1.0 0 -13840069 true "" "plot cuenta-pasajeros-estacion estacion 42"
"Estacion 3" 1.0 0 -7500403 true "" "plot cuenta-pasajeros-estacion estacion 84"
"Estacion 4" 1.0 0 -955883 true "" "plot cuenta-pasajeros-estacion estacion 21"
"Estacion 5" 1.0 0 -6459832 true "" "plot cuenta-pasajeros-estacion estacion 0"
"Estacion 6" 1.0 0 -1184463 true "" "plot cuenta-pasajeros-estacion estacion 63"
